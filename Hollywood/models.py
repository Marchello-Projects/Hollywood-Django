...
class ClinicAddress(models.Model):
    ...
    address = models.CharField(
        _("Адреса"), max_length=255, choices=CLINIC_CHOICES, unique=True
    )
...
class PatientRecord(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="records"
    )
    ...
    visit_reason = models.CharField(
        _("Причина візиту"), max_length=50, choices=VisitReason.choices
    )

    clinic_address = models.ForeignKey(
        ClinicAddress, on_delete=models.PROTECT, verbose_name=_("Адреса поліклініки")
    )
